...
class LoRABenchmark:
    def __init__(self, tensor_config: Dict[str, int]):
        ...
        self.tensor_shapes = {
            # Primary tensors
            'X': (self.M, self.N),  # Input activations
            'W': (self.N, self.P),  # Original weight matrix
            'A': (self.N, self.R),  # LoRA matrix A
            'B': (self.R, self.P),  # LoRA matrix B
            'O': (self.M, self.P),  # Output
            
            # Intermediate tensors (for seq operations)
            'C': (self.M, self.P),  # Intermediate result, same shape as O: X @ W
            'D': (self.M, self.R),  # Intermediate result: X @ A
            'E': (self.M, self.P),  # Intermediate result, same shape as O: D @ B
        }
    ...
    def benchmark_kernel(self, kernel_module, block_sizes: Dict[str, int], warmup_runs: int = 10, benchmark_runs: int = 100) -> float:
        ...
        try:
            ...
            for _ in range(warmup_runs):
                kernel_fn(*args, block_n=block_n, block_p=block_p)
...
def run_comprehensive_benchmark(tensor_configs, block_configs, ir_file, start_expressions, num_expressions, top_k, output_file):
    ...
    for tensor_idx, tensor_config in enumerate(tensor_configs):
        print(f"\nTensor Configuration {tensor_idx + 1}/{len(tensor_configs)}: M={tensor_config['M']}, N={tensor_config['N']}, P={tensor_config['P']}, R={tensor_config['R']}")
        
        # Initialize benchmark with this tensor configuration
        benchmark = LoRABenchmark(tensor_config)
        
        for block_idx, block_config in enumerate(block_configs):
            print(f"  Block Configuration {block_idx + 1}/{len(block_configs)}: block_n={block_config['block_n']}, block_p={block_config['block_p']}")
...
def print_comprehensive_report(all_results, top_k):
    ...
    for config_idx, config_result in enumerate(all_results):
        ...
        print(f"  Tensor Shape: M={tensor_config['M']}, N={tensor_config['N']}, P={tensor_config['P']}, R={tensor_config['R']}")
        print(f"  Block Size: block_n={block_config['block_n']}, block_p={block_config['block_p']}")
    ...
    for i, result in enumerate(overall_best):
        ...
        print(f"   Tensor Config: M={result.tensor_config['M']}, N={result.tensor_config['N']}, P={result.tensor_config['P']}, R={result.tensor_config['R']}")
...
def main():
    ...
    # Configuration
    IR_FILE = "/home/chani227/triton/part4/benchmark_lora/lora_cost3_kern2_index0.txt"
    OUTPUT_FILE = "/home/chani227/triton/part4/benchmark_lora/benchmark_lora_comprehensive.json"
    TENSOR_FILE = "/home/chani227/triton/part4/benchmark_lora/tensor_configs.json"
    BLOCK_FILE = "/home/chani227/triton/part4/benchmark_lora/block_configs.json"
    START_EXPRESSIONS = 0
    ...
    # BLOCK_CONFIGS = [
    #     {'block_n': 16, 'block_p': 16},    # Small blocks
    #     {'block_n': 32, 'block_p': 32},    # Medium blocks
    #     {'block_n': 64, 'block_p': 64},    # Large blocks
    #     {'block_n': 16, 'block_p': 32},    # Mixed blocks
    #     {'block_n': 32, 'block_p': 16},    # Mixed blocks (reversed)
    # ]
    with open(BLOCK_FILE, 'r') as f:
        BLOCK_CONFIGS = json.load(f)

    parser = argparse.ArgumentParser(description="Run comprehensive LoRA IR benchmarks")
    ...
    print("Starting comprehensive LoRA benchmarks...")
