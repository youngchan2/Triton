...
class AttaccBenchmark:
    def __init__(self, tensor_config: Dict[str, int]):
        ...
        self.tensor_shapes = {
            ...
            'V': (1, self.M, self.D),
            'C': (1, self.M, self.P),
            'C_exp': (1, self.M, self.P),
            'C_sum': (1, self.M),
            'C_div': (1, self.M, self.P),
            'O': (1, self.M, self.D),
            'O1': (self.M, 1, self.D)
        }
    ...
    def benchmark_kernel(self, kernel_module, block_sizes: Dict[str, int], warmup_runs: int = 10, benchmark_runs: int = 100) -> float:
        ...
        try:
            ...
            for _ in range(warmup_runs):
                kernel_fn(*args, block_k=block_k, block_p=block_p)
...
def print_comprehensive_report(all_results, top_k):
    ...
    for config_idx, config_result in enumerate(all_results):
        ...
        print(f"  Tensor Shape: M={tensor_config['M']}, N={tensor_config['N']}, D={tensor_config['D']}, H={tensor_config['H']}, P={tensor_config['P']}")
        print(f"  Block Size: block_p={block_config['block_p']}, block_k={block_config['block_k']}")
    ...
    for i, result in enumerate(overall_best):
        ...
        print(f"   Tensor Config: M={result.tensor_config['M']}, N={result.tensor_config['N']}, D={result.tensor_config['D']}, H={result.tensor_config['H']}, P={result.tensor_config['P']}")
...
def main():
    ...
    OUTPUT_FILE = "./benchmark_llama/benchmark_llama_4090.json"
